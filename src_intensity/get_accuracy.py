...
def get_files(combination):
    method = sys.argv[1]
    intensity = combination
    base_path = '/home/sadayuki/Dropbox/Data-Mining-Research/'
    manual_file = base_path + 'data_intensity/sep_' + method + '/manual_classification/' + intensity
    new_method_file = base_path + 'data_intensity/sep_' + method + '/new_method/' + intensity
    out_file = base_path + 'data_intensity/accuracy/'+ method + '/' + intensity
    ...
    for human, system in zip(human_emo_list, after_news_emo_list):
        ...
        for system_emo in system_emos:
            if system_emo in human_emos:
                print(human[0], system[0])
                print(system_emo, 'in included in')
                print(human_emos)
                included += 1
...
def creat_file(precision_easy, precision, precision_each_emo, recall, recall_each_emo, out_file):
    with open(out_file, 'w+') as f:
        ...
        for emo, stat_dict in precision_each_emo.items():
            ...
            f.write(str(stat_dict['match']) + ' / ' + str(stat_dict['total']) + '\n')
        f.write('\n')
...
def get_accuracy(combination):
    manual_file, new_method_file, out_file = get_files(combination)
...
def get_combination_list():
    return ['all', 'strong', 'strong_med', 'strong_weak', 'med', 'med_weak', 'weak']

if __name__ == '__main__':
    combination_list = get_combination_list()
    for combination in combination_list:
        get_accuracy(combination)
